...
// define paths
// --------------------------------------------------------------------------
define('PATH_TO_THUMBS', 'uploads/thumbnail/');
define('PATH_TO_LOGS',   '');
...
function __construct() {
	//$this->_logic($src,$a,$w,$h);
}

public function logic($src,$a,$w,$h) {
	// --------------------------------------------------------------------------
	// what this program is supposed to do
	// --------------------------------------------------------------------------
		
	$this->pathToImage = "uploads/".$src;
	if (!file_exists($this->pathToImage)) {
		trigger_error('input image not found  at '. $this->pathToImage, E_USER_ERROR);
	}
		
	$this->thumbArea   = isset($a) ? $a : null ;
	$this->thumbWidth  = isset($w) ? $w : null ;
	$this->thumbHeight = isset($h) ? $h : null ;
