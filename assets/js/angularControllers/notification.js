...
app.controller('AppCtrl', function($scope, $cordovaPush, $cordovaDialogs, $cordovaMedia, $cordovaToast, ionPlatform, $http) {
    ...
    $scope.reg_update = function($event){
    ...
    $scope.register = function () {
        ...
        if (ionic.Platform.isAndroid()) {
            config = {
                "senderID": "474754079944" // REPLACE THIS WITH YOURS FROM GCM CONSOLE - also in the project URL like: https://console.developers.google.com/project/434205989073
            };
        }
    ...
    function storeDeviceToken(type) {
        ...
        $scope.push_data = { user_token: $scope.regId, type: type };

        $http({
				method: 'POST',
				url: get_base_url()+'/notification/register_push',
				data: $.param($scope.push_data),
				headers: {'Content-Type' : 'application/x-www-form-urlencoded'}
			})

			.success(function(data){
				if(!data.success){
					$cordovaToast.showShortCenter(data.errors.systemError);
				} else {
					$cordovaToast.showShortCenter(data.success.message);
				}
			});
    }
...
app.controller('NotificationListController', function($scope, $http){
